    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between py-3">
                <div>
                    <h4 class="page-title mb-1 text-dark fw-bold">Edit Product: {{ $edit_data->name }}</h4>
                    <p class="text-muted font-size-13 mb-0">Update catalog details, media, variants, pricing and publishing state from one screen.</p>
                </div>
        ...
        <div class="row">
            <div class="col-lg-8">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="section-title d-flex justify-content-between align-items-center">
                            <span><i class="fe-maximize me-1"></i> Size Chart <small class="text-muted">(ঐচ্ছিক — ল্যান্ডিং পেজে দেখাবে)</small></span>
                            <button type="button" class="btn btn-sm btn-success add-size-chart-row rounded-pill px-3"><i class="fa fa-plus me-1"></i> Add Row</button>
                        </div>
                        <div id="size-chart-wrapper">
                            @forelse($sizeChartRows as $key => $row)
                                <div class="variant-card size-chart-row">
                                    <div class="row align-items-end">
                                        <div class="col-md-3 mb-2">
                                            <label class="form-label">Size</label>
                                            <input type="text" name="size_chart[{{ $key }}][size]" class="form-control" value="{{ $row['size'] ?? '' }}">
                                        </div>
                                        <div class="col-md-3 mb-2">
                                            <label class="form-label">Chest (inch)</label>
                                            <input type="text" name="size_chart[{{ $key }}][chest]" class="form-control" value="{{ $row['chest'] ?? '' }}">
                                        </div>
                                        <div class="col-md-3 mb-2">
                                            <label class="form-label">Length (inch)</label>
                                            <input type="text" name="size_chart[{{ $key }}][length]" class="form-control" value="{{ $row['length'] ?? '' }}">
                                        </div>
                                        <div class="col-md-3 mb-2">
                                            <button type="button" class="btn btn-danger btn-remove-size-chart w-100"><i class="fa fa-trash"></i></button>
                                        </div>
                                    </div>
                                </div>
                            @empty
                                <div class="variant-card size-chart-row">
                                    <div class="row align-items-end">
                                        <div class="col-md-3 mb-2">
                                            <label class="form-label">Size</label>
                                            <input type="text" name="size_chart[0][size]" class="form-control" placeholder="e.g. M">
                                        </div>
                                        <div class="col-md-3 mb-2">
                                            <label class="form-label">Chest (inch)</label>
                                            <input type="text" name="size_chart[0][chest]" class="form-control" placeholder="e.g. 38">
                                        </div>
                                        <div class="col-md-3 mb-2">
                                            <label class="form-label">Length (inch)</label>
                                            <input type="text" name="size_chart[0][length]" class="form-control" placeholder="e.g. 27">
                                        </div>
                                        <div class="col-md-3 mb-2">
                                            <button type="button" class="btn btn-success add-size-chart-row w-100"><i class="fa fa-plus"></i></button>
                                        </div>
                                    </div>
                                </div>
                            @endforelse
                        </div>
                        <small class="text-muted">Unit: inch. শুধু Size লিখলেই রো সেভ হবে; Chest/Length খালি রাখা যাবে।</small>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $("body").on("click", ".add-size-chart-row", function () {
        let wrapper = $("#size-chart-wrapper");
        let newRow = wrapper.find(".size-chart-row").first().clone();

        newRow.find('input').each(function () {
            let oldName = $(this).attr('name');
            $(this).attr('name', oldName.replace(/\[\d+\]/, '[' + sizeChartIndex + ']'));
            $(this).val('');
        });

        newRow.find('.add-size-chart-row')
            .removeClass('btn-success add-size-chart-row')
            .addClass('btn-danger btn-remove-size-chart')
            .html('<i class="fa fa-trash"></i>');
    ...
    $("body").on("click", ".btn-remove-size-chart", function () {
        $(this).parents(".size-chart-row").remove();
    });
