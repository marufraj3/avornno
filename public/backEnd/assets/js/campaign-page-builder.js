...
(function () {
    ...
    const WIDGETS = {
        ...
        gallery: {
            label: 'Photo gallery', icon: 'fe-image', category: 'media',
            defaults: { title: 'ছবি দেখুন' },
            fields: [{ key: 'title', label: 'শিরোনাম' }],
            baseStyle: { padding: '10px', color: '#172033' },
            render: (w, editor) => `<div><h2 style="text-align:center;margin-bottom:20px">${editable(w, 'title', editor)}</h2><div class="cpb-gallery-preview-grid" data-cpb-dynamic="gallery">${galleryPreview()}</div></div>`
        },
        sizechart: {
            label: 'Size chart', icon: 'fe-maximize', category: 'conversion',
            defaults: { title: 'Size Chart', button: 'সাইজ মেসারমেন্ট দেখুন' },
            fields: [{ key: 'title', label: 'শিরোনাম' }, { key: 'button', label: 'টগল বাটনের লেখা' }],
            baseStyle: { padding: '10px', color: '#172033' },
            render: (w, editor) => `<div><h2 style="text-align:center;margin-bottom:16px">${editable(w, 'title', editor)}</h2><div class="cpb-sizechart-holder" data-cpb-dynamic="sizechart"><div class="cpb-sizechart-toggle-preview">${editable(w, 'button', editor)} <span>▾</span></div>${sizeChartPreview()}</div></div>`
        },
        trust: {
...
`;
    ...
    function galleryPreview() {
        const sources = [];
        (Array.isArray(context.products) ? context.products : []).forEach(product => {
            if (safeUrl(product.image)) sources.push({ image: product.image, name: product.name });
        });
        (Array.isArray(context.reviews) ? context.reviews : []).forEach(review => {
            if (safeUrl(review.image)) sources.push({ image: review.image, name: 'Gallery' });
        });
        const list = sources.slice(0, 8);
        if (!list.length) return '<div class="cpb-image-placeholder" style="grid-column:1/-1">প্রোডাক্টের ছবি যোগ করলে গ্যালারিতে দেখাবে</div>';
        // <figure> CampaignPageSanitizer-এর allowedTags-এ নেই, তাই <div> ব্যবহার করা হলো
        return list.map(item => `<div class="cpb-gallery-preview"><img src="${attr(safeUrl(item.image))}" alt="${attr(item.name)}"><span>⌕ বড় করে দেখুন</span></div>`).join('');
    }

    function sizeChartPreview() {
        return `<div class="cpb-sizechart-preview"><p style="margin:0 0 8px;font-size:12px;color:#64748b;font-weight:700">Unit: inch</p>
            <table style="width:100%;border-collapse:collapse;font-size:13px">
                <thead><tr>${['Size','Chest','Length'].map(h => `<th style="border:1px solid #e2e8f0;padding:8px;background:#f1f5f9">${h}</th>`).join('')}</tr></thead>
                <tbody>${[['M','38','27'],['L','40','28'],['XL','42','29']].map(row => `<tr>${row.map((cell, i) => `<t${i===0?'h':'d'} style="border:1px solid #e2e8f0;padding:8px;text-align:center">${cell}</t${i===0?'h':'d'}>`).join('')}</tr>`).join('')}</tbody>
            </table>
            <p style="margin:8px 0 0;font-size:11.5px;color:#94a3b8">আসল মান প্রোডাক্ট এডিট → Size Chart থেকে আসবে</p></div>`;
    }
