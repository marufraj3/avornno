...
class FacebookCapiService
{
    ...
    /**
     * ফোন নম্বরকে Facebook-এর চাওয়া international format-এ নিয়ে আসে।
     *
     * Facebook CAPI চায় country code সহ, leading zero / '+' / space ছাড়া নম্বর
     * (যেমন BD: 8801712345678)। কিন্তু আমাদের চেকআউট ফর্ম 11 ডিজিটের লোকাল
     * ফরম্যাটে (017xxxxxxxx) নম্বর পাঠায়। ওভাবে hash করলে Facebook কাস্টমারকে
     * মিলিয়ে নিতে পারে না → Event Match Quality কমে যায় → অ্যাড অপ্টিমাইজেশন
     * খারাপ হয়। তাই hash করার আগে নম্বরটা ঠিক করে নিই।
     */
    protected function normalizePhone($phone)
    {
        ...
        // 00 দিয়ে শুরু হলে ওটা country code prefix, শুধু 00 বাদ দিই (0088017... -> 88017...)
        if (strpos($phone, '00') === 0) {
            $phone = ltrim($phone, '0');
        }

        // বাংলাদেশি লোকাল মোবাইল ফরম্যাট: 0 + 10 ডিজিট (যেমন 01712345678)
        // -> শুধু 88 যোগ করলেই international format: 88 + 01712345678 = 8801712345678
        // (BD country code = 880, তাই লোকাল নম্বরের 0 টা এখানেই থেকে যায়।
        //  leading zero কেটে দিলে 881712345678 হয়ে যেত — ভুল নম্বর।)
    ...
    protected function prepareCustomData($data, $eventName)
    {
        ...
        // Content type ('product' / 'product_group') — কল সাইটগুলো পাঠায়,
        // আগে prepareCustomData এটা payload-এ তুলত না, ফলে চুপচাপ বাদ পড়ে যেত।
        if (isset($data['content_type'])) {
            $prepared['content_type'] = $data['content_type'];
        }
