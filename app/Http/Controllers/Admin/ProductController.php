...
class ProductController extends Controller
{
    function __construct()
    {
        ...
        $this->middleware('permission:product-create', ['only' => ['create','store','duplicate']]);
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            ...
            'size_chart.*.size'   => 'nullable|string|max:30',
            'size_chart.*.chest'  => 'nullable|string|max:30',
            'size_chart.*.length' => 'nullable|string|max:30',
        ]);
    ...
    // ================================
    /**
     * Create an independent copy of a product and all product-specific relations.
     * Product files are copied too, so removing a file from either product does not
     * break the other one.
     */
    public function duplicate($id)
    {
        ...
        try {
            $product = DB::transaction(function () use ($source, &$copiedFiles) {
                $product = $source->replicate(['slug', 'product_code', 'digital_file']);
                $product->slug = Str::slug($source->name) . '-copy-' . Str::lower(Str::random(8));
                $product->product_code = 'P' . now()->format('ymdHis') . Str::upper(Str::random(4));

                // Copy file-backed fields before the new record is written.
                $product->meta_image = $this->duplicatePublicFile($source->meta_image, $copiedFiles);
                $product->pro_video_path = $this->duplicatePublicFile($source->pro_video_path, $copiedFiles);
                $product->digital_file = $this->duplicatePrivateFile($source->digital_file, $copiedFiles);
                $product->save();

                $product->sizes()->sync($source->sizes->pluck('id')->all());
                $product->colors()->sync($source->colors->pluck('id')->all());

                foreach ($source->images as $image) {
                    Productimage::create([
                        'product_id' => $product->id,
                        'image' => $this->duplicatePublicFile($image->image, $copiedFiles),
                        'color_id' => $image->color_id,
                        'size_id' => $image->size_id,
                    ]);
                }

                foreach ($source->variantPrices as $variant) {
                    ProductVariantPrice::create([
                        'product_id' => $product->id,
            ...
            // A database transaction cannot roll back filesystem writes.
            ...
            report($exception);
            Toastr::error('Product could not be duplicated. Please try again.');
    ...
    /**
     * Duplicate a public product asset while retaining the path format used in the
     * database. Some older uploads live under base_path(), while others live below
     * public_path(), so both locations are supported.
     */
    private function duplicatePublicFile(?string $path, array &$copies): ?string
    {
        if (!$path) {
            return null;
        }

        if (isset($copies['public:' . $path])) {
            return $copies['public:' . $path]['path'];
        }

        foreach ([base_path($path), public_path($path)] as $source) {
            if (!is_file($source)) {
                continue;
            }

            $extension = pathinfo($source, PATHINFO_EXTENSION);
            $filename = pathinfo($source, PATHINFO_FILENAME) . '-copy-' . Str::lower(Str::random(10));
            $target = dirname($source) . DIRECTORY_SEPARATOR . $filename . ($extension ? '.' . $extension : '');

            if (!copy($source, $target)) {
                throw new \RuntimeException('Unable to copy product asset.');
            }

            $newPath = rtrim(dirname($path), '/\\') . '/' . basename($target);
            $copies['public:' . $path] = ['path' => $newPath, 'file' => $target];
            return $newPath;
        }

        // Keep legacy references usable if their original file is no longer present.
        return $path;
    }

    /** Duplicate a private digital download, if the product has one. */
    private function duplicatePrivateFile(?string $path, array &$copies): ?string
    {
        if (!$path || !Storage::disk('private')->exists($path)) {
            return $path;
        }

        if (isset($copies['private:' . $path])) {
            return $copies['private:' . $path]['path'];
        }

        $extension = pathinfo($path, PATHINFO_EXTENSION);
        $newPath = trim(dirname($path), '.') . '/' . pathinfo(basename($path), PATHINFO_FILENAME)
            . '-copy-' . Str::lower(Str::random(10)) . ($extension ? '.' . $extension : '');

        if (!Storage::disk('private')->copy($path, $newPath)) {
            throw new \RuntimeException('Unable to copy digital product file.');
        }

        $copies['private:' . $path] = [
            'path' => $newPath,
            'file' => Storage::disk('private')->path($newPath),
        ];
     ...
     */
    /**
     * Size chart রো গুলো পরিষ্কার করে অ্যারে হিসেবে রিটার্ন করে।
     *
     * অ্যাডমিন ফর্ম থেকে size_chart[0][size] / [chest] / [length] আসে। খালি রো
     * বাদ পড়ে; একটিও রো না থাকলে null সেট হয় (তখন ল্যান্ডিং পেজে সেকশন দেখাবে না)।
     *
     * ⚠️ এখানে json_encode করা যাবে না — Product মডেলে 'size_chart' => 'array'
     * cast আছে, আর Laravel-এর setAttribute() json-castable অ্যাট্রিবিটে
     * castAttributeAsJson() চালায় (Json::encode)। অর্থাৎ আগে থেকে encode করা
     * স্ট্রিং দিলে সেটা দ্বিতীয়বার encode হয়ে ডেটাবেসে
     * "\"[{\\\"size\\\":...}] "\" এমন ডাবল-এনকোডেড মান বসত, এবং পড়ার সময়
     * অ্যারের বদলে স্ট্রিং ফিরত — ফলে Size Chart সেকশন আর দেখাত না।
     */
    private function normalizeSizeChart($rows): ?array
    {
        if (!is_array($rows)) {
            return null;
        }
        ...
        foreach ($rows as $row) {
            ...
            // সাইজের নাম ছাড়া রো অর্থহীন
            if ($size === '') {
                continue;
            }
            $clean[] = ['size' => $size, 'chest' => $chest, 'length' => $length];
        }
