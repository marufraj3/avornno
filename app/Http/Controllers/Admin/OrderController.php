...
class OrderController extends Controller
{
    ...
    */
    ...
    public function fraudCheck(Request $request, FraudCheckService $fraudCheckService)
    {
        ...
        // ম্যানুয়াল চেক বলে লাইভ (cache=off) ডেটা আনা হয়।
        return response()->json(
            $fraudCheckService->checkPhone($request->input('mobile'), 20, true)
        );
    }
    ...
    public function manualFraudCheck(Request $request, FraudCheckService $fraudCheckService)
    {
        ...
        // ম্যানুয়াল চেক — Hoorin API থেকে লাইভ (cache=off) ডেটা আনা হয়
    ...
    */
    ...
    public function index($slug, Request $request)
    {
        if ($slug == 'all') {
            ...
            $this->applyOrderKeyword($show_data, $request->keyword);
            $this->applyTrafficSourceFilter($show_data, $request->traffic_source);
            $show_data = $show_data->paginate(20)->appends($request->query());
        } else {
        ...
        if (!is_object($orderstatus) || !method_exists($orderstatus, 'first')) {
            $orderstatus = OrderStatus::orderBy('id')->get();
        }

        if ($request->ajax()) {
            return view('backEnd.order._table', compact('show_data'));
        }

        $confirmStatus = $orderstatus->first(function ($s) {
            return str_contains(strtolower(($s->slug ?? '') . ' ' . ($s->name ?? '')), 'confirm');
        });
        $cancelStatus = $orderstatus->first(function ($s) {
            return str_contains(strtolower(($s->slug ?? '') . ' ' . ($s->name ?? '')), 'cancel');
        });

        $traffic_source_options = $this->trafficSourceOptions();

        return view('backEnd.order.index', compact(
            'show_data', 'order_status', 'users', 'steadfast', 'pathaostore', 'pathaocities',
            'blockedIps', 'pathao_info', 'redx_info', 'redxAreas', 'redxPickupStores', 'orderstatus',
            'confirmStatus', 'cancelStatus', 'slug', 'traffic_source_options'
        ));
    }

    protected function applyOrderKeyword($query, $keyword)
    {
    ...
    /**
     * অর্ডার লিস্টের ট্র্যাফিক উৎস ড্রপডাউন।
     */
    public function trafficSourceOptions(): array
    {
        return [
            ''          => 'সব ট্র্যাফিক',
            'direct'    => 'Direct',
    ...
    /**
     * ট্র্যাফিক উৎস ফিল্টার। কলাম না থাকলে চুপচাপ স্কিপ করে।
     */
    protected function applyTrafficSourceFilter($query, $source)
    {
        ...
        if ($source === 'direct') {
            $query->where(function ($q) {
                $q->whereNull('traffic_source')->orWhere('traffic_source', '')->orWhere('traffic_source', 'direct');
            });
            return;
        }
    ...
    public function globalSearch(Request $request)
    {
        ...
        $orders = Order::query()
            ->with(['shipping:id,order_id,name,phone', 'status:id,name,slug'])
            ->where(function ($query) use ($q) {
                $query->where('invoice_id', 'LIKE', '%'.$q.'%')
                    ->orWhereHas('shipping', function ($sub) use ($q) {
            ...
            ->latest('id')
            ->limit(8)
            ->get(['id', 'invoice_id', 'order_status', 'amount', 'created_at']);

        return response()->json($orders->map(function ($order) {
            return [
                'invoice' => $order->invoice_id,
                'name' => optional($order->shipping)->name ?: 'Guest',
    ...
    */
    ...
    /**
     * অর্ডার কুইক ভিউ — মডালের জন্য রেন্ডার করা HTML রিটার্ন করে।
     * {order} হিসেবে order id অথবা invoice_id দুটোই কাজ করে।
     */
    public function quickView($order)
    {
        $query = Order::with([
            'shipping', 'payment', 'status', 'customer',
            'orderdetails.size', 'orderdetails.color', 'orderdetails.image',
        ]);

        $model = ctype_digit((string) $order)
            ? (clone $query)->where('id', $order)->first()
            : null;

        if (!$model) {
            $model = $query->where('invoice_id', $order)->first();
        }

        if (!$model) {
            return response()->json(['status' => 'error', 'message' => 'অর্ডার খুঁজে পাওয়া যায়নি'], 404);
        }
        ...
        $html = view('backEnd.order.partials.quick_view_content', [
            'order'       => $model,
            'blockedIps'  => $blockedIps,
            'steadfast'   => $steadfast,
            'pathao_info' => $pathao_info,
            'redx_info'   => $redx_info,
        ])->render();
    ...
    /**
     * পুরোনো JSON কুইক ভিউ (অন্য স্ক্রিনে ব্যবহৃত হলে যেন না ভাঙে)।
     */
    public function quickViewJson($invoice_id)
    {
        $order = Order::where('invoice_id', $invoice_id)
    ...
    public function process($invoice_id)
    {
        $data = Order::where(['invoice_id' => $invoice_id])
            ->with(['orderdetails', 'orderdetails.size', 'orderdetails.color', 'orderdetails.image', 'shipping'])
            ->firstOrFail();
