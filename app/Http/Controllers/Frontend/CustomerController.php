...
class CustomerController extends Controller
{
    ...
    function __construct(FacebookCapiService $facebookCapiService)
    {
        ...
        $this->middleware('customer', ['except' => [
            ...
            'order_save','order_success','order_upsell','order_track','order_track_result'
        ]]);
    }
    ...
    public function signin(Request $request)
    {
        ...
        // Preserve historical vendor/reseller rows but explicitly deny runtime authentication.
        $legacyEmail = $isPhone ? Customer::where('phone', $login)->value('email') : $login;
        $retiredAccount = $legacyEmail
            ? \App\Models\User::where('email', $legacyEmail)
                ->where(function ($query) {
                    $query->whereIn('role', ['reseller', 'vendor'])
                        ->orWhereHas('roles', fn ($role) => $role->whereRaw('LOWER(name) IN (?, ?)', ['reseller', 'vendor']));
                })->exists()
            : false;

        if ($retiredAccount) {
            Toastr::error('This portal is no longer available.', 'Access disabled');
    ...
    public function forgot_verify(Request $request)
    {
        ...
        if ($customer && $this->isRetiredAccountEmail($customer->email)) {
            Toastr::error('This portal is no longer available.');
            return back();
        }
    ...
    public function forgot_resend(Request $request)
    {
        ...
        if ($userType === 'customer') {
            ...
            if ($account && $this->isRetiredAccountEmail($account->email)) $account = null;
            $name = $account?->name;
        } else {
            $account = null;
            $name = null;
    ...
    public function forgot_store(Request $request)
    {
        ...
        if ($userType === 'customer') {
            $customer = Customer::where('phone', $phone)->first();
            if (!$customer || $this->isRetiredAccountEmail($customer->email) || (string) $customer->forgot !== (string) $request->otp) {
    ...
    private function isRetiredAccountEmail(?string $email): bool
    {
 ...
 // === Customer SMS ===
    ...
    public function order_success($id)
    {
        $order = Order::with(['orderdetails.size', 'orderdetails.color', 'shipping'])
            ->where('id', $id)
            ->firstOrFail();
        $orderedIds = $order->orderdetails->pluck('product_id')->all();
        $upsells = \App\Models\OrderBump::with('product.image')
            ->where('status', 1)
            ->whereNotIn('product_id', $orderedIds ?: [0])
            ->orderBy('sort_order')
            ->limit(2)
            ->get()
            ->filter(fn ($bump) => $bump->product)
            ->values();
        return view('frontEnd.layouts.customer.order_success', compact('order', 'upsells'));
    }

    public function addUpsell(Request $request, $id)
    {
        $order = Order::with('orderdetails')->findOrFail($id);
        $bump = \App\Models\OrderBump::with('product')->where('status', 1)->findOrFail($request->bump_id);
        if (!$bump->product) {
            return back()->withErrors(['upsell' => 'অফারটি পাওয়া যায়নি।']);
        }
        if ($order->orderdetails->contains('product_id', $bump->product_id)) {
            return back();
        }
        ...
        OrderDetails::create([
            ...
            'purchase_price' => $bump->product->purchase_price ?? 0,
            'sale_price' => $price,
            'product_discount' => $bump->savings(),
            'qty' => 1,
        ]);
        $order->amount = (float) $order->amount + $price;
        $order->save();
        $bump->increment('conversions');
        Toastr::success('অফারটি আপনার অর্ডারে যোগ হয়েছে।', 'Success');
