...
class AdminMiddleware
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        // ✅ Blocklist: retired vendor/reseller accounts (customer allow)
        // Retired vendor/reseller accounts remain stored but cannot enter any runtime panel.
        if (in_array('reseller', $spatieRoles) || in_array('vendor', $spatieRoles)) {
            Auth::guard('admin')->logout();
            $request->session()->invalidate();
            $request->session()->regenerateToken();
            return redirect()->route('login')->with('error', 'This portal is no longer available.');
        }
